...
- **기능**: BCrypt 암호화, 세션 유지, 자동 로그인

### 2. 대시보드 📊
- **통계 카드**: 총 프로젝트, 활성 프로젝트, 할당된 태스크, 진행 중인 태스크, 마감 임박 태스크
- **클릭 가능한 툴팁**: 프로젝트/태스크 항목 클릭 시 해당 위치로 이동
- **진행 중인 태스크**: 실시간으로 in-progress 상태 태스크 표시

### 3. 프로젝트 관리 📁
- 대시보드에서 "프로젝트 생성" 버튼 클릭
- 프로젝트 카드 클릭 → 칸반 보드 및 태스크 관리
- 프로젝트 상태 관리 (PLANNING, ACTIVE, COMPLETED 등)

### 4. 태스크 관리 ✅
- **칸반 보드**: TODO, IN_PROGRESS, IN_REVIEW, DONE 컬럼
- **태스크 생성**: 제목, 설명, 우선순위, 담당자, 마감일 설정
- **태스크 상세**: 체크리스트, 댓글, 첨부파일 관리
- **실시간 업데이트**: 태스크 삭제 시 칸반 보드 즉시 업데이트
...
2. **태스크 관리 개선**
   - 태스크 삭제 시 칸반 보드 즉시 업데이트 (새로고침 불필요)
   - 불필요한 프로젝트 레이블 기능 제거
   - 시간 추적 기능 제거 (더 심플한 UI)
